...
export class SupabaseClient {
    ...
    async validateSession() {
        ...
        try {
            ...
            console.log('[SupabaseClient] 세션 만료됨, 토큰 갱신 시도...');
    ...
    async refreshSession() {
        ...
        try {
            ...
            console.log('[SupabaseClient] 토큰 갱신 성공');
            return { success: true };
        } catch (error) {
    ...
    async saveCollectionLog(logData) {
        ...
        try {
            await fetch(`${this.supabaseUrl}/rest/v1/collection_logs`, {
                method: 'POST',
                headers: {
                    'apikey': this.supabaseKey,
                    'Authorization': `Bearer ${this.session.access_token}`,
            ...
            console.log('[SupabaseClient] 수집 로그 저장 완료:', logData.status);
        } catch (error) {
            console.error('[SupabaseClient] 수집 로그 저장 실패:', error);
        }
    }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.length > 0) {
                    ...
                    await chrome.storage.local.set({ supabaseSession: this.session });
                    console.log('[SupabaseClient] 전송 한도 차감 완료:', newLimit);
                }
            }
        } catch (error) {
