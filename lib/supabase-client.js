...
export class SupabaseClient {
    ...
    async setSession(sessionData) {
        ...
        console.log('[SupabaseClient] Session synchronized from web');
    ...
    async saveProduct(product) {
        ...
        */
        ...
        try {
            ...
            const endpoint = isUpdate ? `${this.supabaseUrl}/rest/v1/products?id=eq.${product.id}` : `${this.supabaseUrl}/rest/v1/products`;
            ...
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    ...(isUpdate ? {} : { user_id: this.session.user.id }), // 필수: 사용자 ID (Insert시에만)
                    ...
                    // Price Info
                    price: product.price,
                    currency: product.currency || 'KRW', // Add currency field
                    cost: 0,
            ...
            const productId = isUpdate ? product.id : (insertedData && insertedData.length > 0 ? insertedData[0].id : null);
    ...
    async checkPlatformActive(platformId) {
        ...
        try {
            // [STRICT MODE] 사용자의 모든 플랫폼 설정을 가져와서 메모리에서 매칭
            // 특정 ID 쿼리 대신 전체를 로드하여 대소문자 이슈 등 방지
            const response = await fetch(
                `${this.supabaseUrl}/rest/v1/sourcing_platform_settings?select=platform_id,is_active`,
                {
                    method: 'GET',
                    headers: {
                        'apikey': this.supabaseKey,
            ...
            if (response.ok) {
                ...
                if (settings && settings.length > 0) {
                    // 메모리에서 Case-Insensitive 매칭
                    const targetSetting = settings.find(s =>
                        s.platform_id && s.platform_id.toLowerCase().trim() === platformId.toLowerCase().trim()
                    );

                    if (targetSetting) {
                        return {
                            isActive: targetSetting.is_active === true,
                            isListed: true
                        };
                    }
                }
            } else {
                console.error('[SupabaseClient] 플랫폼 설정 조회 실패:', response.status);
            }
        } catch (error) {
            console.error('[SupabaseClient] 플랫폼 상태 체크 오류:', error);
        }

        // [STRICT] DB에 없으면 비활성 처리 (사용자 요청: "DB에 있는 경우만")
        // 별도의 Fallback 없이 false 반환
        return { isActive: false, isListed: false };
    }
}
