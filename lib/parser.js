...
class ProductParser {
    ...
    async scrollToLoadContent() {
        console.log('🔄 Lazy Loading 콘텐츠 로드를 위해 스크롤 시작...');

        const totalHeight = document.body.scrollHeight;
        const viewportHeight = window.innerHeight;
        let currentScroll = 0;
        const steps = 5; // 스크롤 횟수
        ...
        if (sel && sel.description) {
            for (const selector of sel.description) {
                ...
                if (el) {
                    console.log(`📍 상세 설명 영역 발견: ${selector}, 스크롤 이동`);
                    el.scrollIntoView({ behavior: 'smooth', block: 'start' });
                    await new Promise(resolve => setTimeout(resolve, 1000)); // 상세 설명 로딩 대기
