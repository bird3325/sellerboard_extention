...
class SourcingAnalyzer {
    ...
    // 실시간 표준 환율 매핑 정보 (원화 환산용)
    static EXCHANGE_RATES = {
        USD: 1380, // USD/KRW
        CNY: 195,  // CNY/KRW
        EUR: 1500, // EUR/KRW
        JPY: 9.2,  // JPY/KRW (100엔당 ~920원)
        KRW: 1
    };

    /**
     * 해외 통화(USD, CNY 등)를 원화(KRW) 소싱 원가로 환율 변환합니다.
     */
    static convertToKRW(price, currency = 'KRW') {
        const p = parseFloat(price) || 0;
        if (p <= 0) return 0;

        const curr = (currency || 'KRW').toUpperCase();
        let rate = this.EXCHANGE_RATES[curr] || 1;

        // 화폐 단위가 KRW로 수집되었으나 가격이 300 이하인 달러 유추 패턴 대처 (예: $12.50)
        if (curr === 'KRW' && p < 300) {
            rate = this.EXCHANGE_RATES.USD;
        }
    ...
    static analyzeItem(item, criteria = {}) {
        ...
        // 1. 마진율 및 마진 시뮬레이션
        // 알리익스프레스/해외소싱 수집 가격을 환율(USD=1,380원, CNY=195원 등) 반영하여 '원화(KRW) 실제 소싱 원가'로 변환합니다.
        const rawPrice = item.cost || parseFloat(item.price) || 0;
        const currency = item.currency || 'KRW';
        const actualCostKRW = this.convertToKRW(rawPrice, currency);

        // 국내 예상 판매가는 원화 소싱 원가 기준 2.5배 배율(기본값)을 적용하여 시뮬레이션합니다.
        const multiplier = criteria.multiplier || 2.5; 
        const sellingPrice = actualCostKRW * multiplier;

        const marginResult = this.calculateMargin(sellingPrice, actualCostKRW, {
        ...
        return {
            ...
            costPrice: marginResult.costPrice,
            sellingPrice: marginResult.sellingPrice,
            fee: marginResult.fee,
            shippingCost: marginResult.shippingCost,
            rawPrice: rawPrice,
            currency: currency,
            reason
        };
     ...
     * @returns {Object} { marginRate, netProfit, costPrice, sellingPrice, fee, shippingCost }
     */
    static calculateMargin(sellingPrice, costPrice, options = {}) {
        ...
        const netProfit = Math.round(sellingPrice - totalExpense);
...
if (typeof window !== 'undefined') window.SourcingAnalyzer = SourcingAnalyzer;
if (typeof globalThis !== 'undefined') globalThis.SourcingAnalyzer = SourcingAnalyzer;
if (typeof self !== 'undefined') self.SourcingAnalyzer = SourcingAnalyzer;
